...
template<typename T>
void sorted_list<T>::print_rang_list() const {
  if(!empty() && size()>=3){
    Node<T>* temp1=this->head_;
    Node<T>* temp2=this->tail_;
    std::cout << "3 obaveze sa najvećim prioritetom" << std::endl;
    //auto it1=this->begin();
    int i=0;
    while(temp1 && i<3){
      std::cout << temp1->val_ << std::endl;
      temp1=temp1->next_;
      ++i;
    }
    std::cout<<"3 obaveze sa najmanjim prioritetom"<<std::endl;
    //auto it2=(this->end())--;
    int j=0;
    while(temp2 && j<3){
      std::cout << temp2->val_ << std::endl;
      temp2=temp2->prev_;
      ++j;
    }
  } 
  else if(empty()){ std::cout<<"List is empty"<<std::endl;  }  
  else print(); 
}
...
void sorted_list<T>::remove(Iterator it){
  ...
  std::cout << "List is empty!" << std::endl;
    ...
    it.getNode()->next_->prev_=it.getprevNode();  
    delete it.getNode();
    --size_;
